...
const Post = () => {
  ...
  const handleLike = async (postId) => {
    try {
      const response = await axios.put(`${BASE_URL}/post/like-post/?${postId}/`);
        ...
        <div className="md:w-2/3 lg:w-2/5 p-5 md:px-12 md:py-8 h-full mx-auto">
          ...
          
          {
            posts.length === 0 ? (
              ...
              // Display message or image when no posts are available
          ...
          posts.map((post) => (
            <div key={post.id} className="mb-4 border rounded-lg overflow-hidden shadow-md bg-white">
              ...
              <div className="p-3 flex justify-between">
                ...
                <button
                  className={`flex items-center text-gray-500 m-3 hover:text-blue-500 transition-colors ${post.isLiked ? 'text-blue-500' : ''
                    }`}
